...
class CRM_UpdateMembershipRecord {
    ...
    public function updateMembershipStatus( )
    {
        ...
        foreach ( array('Pending','Cancelled','Expired','Deceased') as $status ) {
            $excludeStatuses[] = array_search($status, $allStatus);
        }
        $excludeStatuses = implode(',', $excludeStatuses);
...
WHERE      civicrm_membership.is_test = 0 AND
           civicrm_membership.status_id NOT IN ($excludeStatuses) AND
